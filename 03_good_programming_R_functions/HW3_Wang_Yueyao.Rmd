...
```  
So from we could know, when the tolerance is 1e-5, the biggest negative root of this function is `r beta[length(beta)]`  
...
car_geconstat_select <- fread(
  input = "../02_data_munging_summarizing_R_git/Open_Data_RDW__Geconstateerde_Gebreken.csv", 
...
stargazer(fit1,title = "summary of linear regression",type = "text",align = T)
stargazer(anova(fit1),title = "anova table",type = "text",align = T)
```
...
summary3 <- merged_tidy_2017 %>% group_by(model) %>%
  summarise(number_defect = n_distinct(defect_code)) %>%
  mutate(make = as.numeric(as.factor(model)))
#kable(head(summary3),caption = "number of distinct defects observed by it model") %>%
  kable_styling(full_width = T)
fit2 <- lm(number_defect~model,data = summary3)
stargazer(fit2,title = "summary of linear regression",type = "text",align = T)
stargazer(anova(fit2),title = "anova table",type = "text",align = T)
```
...
```{r Appendix, ref.label=c("Problem6_function","Problem6_mean","Problem6_sd","problem7","Problem8_Plot","Problem8_function"), echo=TRUE, eval=F, tidy=TRUE, include=T}
  
```

### Reference:  

I didn't have a good idea to construct a clean anova table at first and knew about the stargazer function from Erica Porter's work.  
